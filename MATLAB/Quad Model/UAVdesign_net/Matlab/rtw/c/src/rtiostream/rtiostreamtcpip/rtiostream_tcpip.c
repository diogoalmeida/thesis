    ...
    unsigned int timeoutSecs)
{
     ...
     */
    sighandle(pending = select((int)(sock + 1), &ReadFds, NULL, NULL, &tval));
    if (pending == SOCK_ERR) {
    ...
    size_t        *sizeRecvd)
{
    ...
    sighandle(nRead = recv(sock, dst, (int)size, 0U));
...
#ifndef VXWORKS
    sighandle(nSent = send(sock, src, sizeLim, 0));
#else
    /*
     * VXWORKS send prototype does not have src as const.  This suppresses
     * the compiler warning.
     */
    sighandle(nSent = send(sock, (char *)src, sizeLim, 0));
#endif
    if (nSent == SOCK_ERR) {
        retVal = RTIOSTREAM_ERROR;
    } else { 
...
static SOCKET serverOpenConnection(void)
{
    ...
    if ( (pending > 0) && (error==RTIOSTREAM_NO_ERROR) ) {
         ...
         */
#if (!defined(_WIN32)) && (!defined(VXWORKS))
        sighandle(cFd = accept(lFd, (struct sockaddr *)&clientAddr, (socklen_t *) &sFdAddSize));
#else
        sighandle(cFd = accept(lFd, (struct sockaddr *)&clientAddr, &sFdAddSize));
#endif
