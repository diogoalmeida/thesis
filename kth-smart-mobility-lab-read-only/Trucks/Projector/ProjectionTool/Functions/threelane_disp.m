function threelane_disp(A1,A2,A3,scale,roadInfo,jj,i)
dir1=roadInfo(i,4);
dir2=roadInfo(i+1,4);
dir3=roadInfo(i+2,4);
...
Binner1=zeros(3,length(A3(1,1:end-1)));
Wouter=zeros(3,length(A1(1,1:end-1)));
Winner=zeros(3,length(A3(1,1:end-1)));
Bouter2=zeros(3,length(A1(1,1:end-1)));
Binner2=zeros(3,length(A3(1,1:end-1)));
 ...
 for i=1:length(A3(1,:))-1
        point=[A3(1,i+1)-A3(1,i) A3(2,i+1)-A3(2,i) 0];
        orthogonal=cross(point,[0 0 1]);
        orthonormal=orthogonal/norm(orthogonal);
        if dir1==dir2
            Binner1(:,i)=-orthonormal*scale*1*d+[A3(1,i) A3(2,i) 0];
            Winner(:,i)=-orthonormal*scale*k1*d+[A3(1,i) A3(2,i) 0];
            Binner2(:,i)=-orthonormal*scale*1*k2*d+[A3(1,i) A3(2,i) 0];
        else
            Binner1(:,i)=orthonormal*scale*1*d+[A3(1,i) A3(2,i) 0];
            Winner(:,i)=orthonormal*scale*k1*d+[A3(1,i) A3(2,i) 0];
            Binner2(:,i)=orthonormal*scale*1*k2*d+[A3(1,i) A3(2,i) 0];
